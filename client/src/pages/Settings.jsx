        ...
        {selectedMember && (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-lg glass-card !p-0 shadow-2xl overflow-hidden rounded-[32px] md:rounded-[48px] border-none">
              <div className="p-6 md:p-10 bg-slate-900 text-white relative overflow-hidden"><div className="relative z-10 flex items-center gap-4 md:gap-6"><div className="w-16 h-16 md:w-20 md:h-20 rounded-[24px] md:rounded-[28px] bg-white p-1 shadow-2xl"><div className="w-full h-full rounded-[20px] md:rounded-[24px] overflow-hidden bg-slate-100"><img src={selectedMember.avatar ? (selectedMember.avatar.startsWith('http') || selectedMember.avatar.startsWith('data:') ? selectedMember.avatar : `${API_URL.replace('/api', '')}${selectedMember.avatar}`) : `https://api.dicebear.com/7.x/avataaars/svg?seed=${selectedMember.name}`} className="w-full h-full object-cover" /></div></div><div><h3 className="text-xl md:text-2xl font-black tracking-tighter uppercase leading-none mb-1 md:mb-2 italic">{selectedMember.name}</h3><span className="px-3 py-1 bg-brand-600 rounded-lg text-[8px] md:text-[9px] font-black uppercase tracking-widest">{selectedMember.role}</span></div><button onClick={() => setSelectedMember(null)} className="ml-auto w-10 h-10 md:w-12 md:h-12 bg-white/10 hover:bg-white/20 rounded-2xl flex items-center justify-center transition-all"><X size={20} /></button></div></div>
