...
const AdminLearning = () => {
  ...
  return (
    <div className="w-full space-y-4 pb-10 overflow-y-auto custom-scrollbar">
      {/* 1. ADMIN HEADER - COMPACT VERSION */}
      <section className="relative overflow-hidden rounded-[24px] bg-white border border-slate-100 shadow-xl p-4 md:p-6">
         ...
         <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
               <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-100">
                  <button onClick={() => setTab('courses')} className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${tab === 'courses' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-900'}`}>
                     Registry
                  </button>
                  <button onClick={() => setTab('analytics')} className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${tab === 'analytics' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-900'}`}>
                     Analytics
                  </button>
               </div>
               <button 
                  onClick={() => { setEditingCourse(null); setShowModal(true); }}
                  className="h-10 px-6 bg-amber-500 text-slate-900 rounded-xl font-black text-[9px] uppercase tracking-widest shadow-lg hover:bg-slate-900 hover:text-white transition-all flex items-center gap-2"
               >
      ...
      {tab === 'courses' ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                 {courses.map((course) => (
                    <motion.div 
                      ...
                      className="bg-white p-4 rounded-[24px] border border-slate-100 shadow-md group hover:border-amber-500/20 transition-all"
                    >
                       <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                             <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-tight line-clamp-1">{course.title}</h4>
                          ...
                          <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                             <button onClick={() => { setEditingCourse(course); setCourseForm({...course, tags: course.tags.join(', ')}); setShowModal(true); }} className="w-7 h-7 rounded-lg bg-slate-50 text-slate-400 hover:bg-slate-900 hover:text-white transition-all flex items-center justify-center">
                                <Edit size={12} />
                             </button>
                             <button onClick={() => handleDeleteCourse(course.id)} className="w-7 h-7 rounded-lg bg-slate-50 text-rose-300 hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center">
                                <Trash2 size={12} />
                             </button>
                          </div>
                       </div>
                       ...
                       <div className="mt-3 flex items-center justify-between pt-3 border-t border-slate-50">
                          <div className="flex items-center gap-1.5">
                             ...
                             <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">
                                <span className="text-slate-900">{course.assignedTo}</span>
                             </span>
                          </div>
                          <span className="text-[8px] font-black text-emerald-600 uppercase tracking-widest">Live Node</span>
                       </div>
                    </motion.div>
                 ))}
              </div>
           ...
           <div className="lg:col-span-4 space-y-4">
              <div className="glass-card !p-6 border-slate-100 rounded-[28px] bg-slate-900 text-white shadow-xl overflow-hidden relative">
        ...
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-xl overflow-hidden p-6">
           <div className="flex items-center justify-between mb-6">
              ...
              <button className="px-4 h-10 bg-slate-50 rounded-lg text-[9px] font-black uppercase tracking-widest text-slate-900 border border-slate-100 hover:bg-white transition-all flex items-center gap-2">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <div className="p-6 rounded-[24px] bg-slate-50 border border-slate-100">
                 ...
                 <h4 className="text-2xl font-black text-slate-900 tracking-tighter mt-1">{analytics.length}</h4>
              </div>
              <div className="p-6 rounded-[24px] bg-slate-50 border border-slate-100">
                 <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Participation</p>
                 <h4 className="text-2xl font-black text-slate-900 tracking-tighter mt-1">92%</h4>
              </div>
              <div className="p-6 rounded-[24px] bg-amber-500 text-slate-900 shadow-lg">
                 <p className="text-[8px] font-black opacity-60 uppercase tracking-widest">Certificates</p>
                 <h4 className="text-2xl font-black tracking-tighter mt-1">24</h4>
              </div>
           </div>
        ...
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="relative w-full max-w-4xl bg-white rounded-[40px] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] z-10"
            >
               <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                  <div className="flex items-center gap-3">
                     <div className="w-10 h-10 bg-amber-500 rounded-xl flex items-center justify-center shadow-lg shadow-amber-500/20 text-slate-900">
                        <Plus size={20} />
                     </div>
                     <div>
                        <h2 className="text-xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">{editingCourse ? 'Edit Protocol' : 'Deploy Protocol'}</h2>
                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-0.5">Configuration Node</p>
                     </div>
                  </div>
                  <button onClick={() => setShowModal(false)} className="w-8 h-8 bg-white hover:bg-slate-100 rounded-lg flex items-center justify-center transition-all border border-slate-100">
                     <X size={16} />
                  </button>
               </div>

               <form onSubmit={handleSaveCourse} className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                           <div className="space-y-1.5">
                              ...
                              <select 
                                ...
                                className="w-full h-11 px-4 rounded-xl bg-slate-50 border border-slate-100 text-[10px] font-black uppercase tracking-widest outline-none"
                              >
                     ...
                     <div className="space-y-4">
                        ...
                        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 space-y-3">
                           ...
                           <div className="space-y-2 max-h-[140px] overflow-y-auto custom-scrollbar pt-2">
