...
const AdminTimesheets = () => {
  ...
  return (
    <div className="w-full h-full flex flex-col p-1 md:p-10 space-y-6 md:space-y-10 animate-in fade-in duration-1000 overflow-y-auto scrollbar-hide">
      
      {/* Header */}
      <section className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white/40 p-6 md:p-10 rounded-[32px] md:rounded-[48px] border border-gray-100 backdrop-blur-md shadow-xl shadow-slate-900/5">
        <div>
          <h1 className="text-2xl md:text-5xl font-black text-slate-900 tracking-tighter leading-none mb-2">
             TEMPORAL COMMAND
          </h1>
      ...
      <div className="glass-card p-0 overflow-hidden shadow-2xl shadow-slate-900/5">
        <div className="p-6 md:p-8 border-b border-slate-100 flex items-center justify-between bg-gray-50/50">
           <div className="flex items-center gap-4">
              ...
              <h2 className="text-lg md:text-[20px] font-black text-slate-900 tracking-tight">Temporal Records</h2>
           </div>
           <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-slate-100">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/30">
                <th className="p-6 md:p-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Specialist</th>
                <th className="p-6 md:p-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Mission Description</th>
                <th className="p-6 md:p-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Effort</th>
                <th className="p-6 md:p-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Status</th>
              ...
              {filteredEntries.map((entry, idx) => (
                <tr key={entry._id || idx} className="group hover:bg-slate-50/30 transition-colors">
                  <td className="p-6 md:p-8">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl overflow-hidden border-2 border-white shadow-md">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{new Date(entry.date).toLocaleDateString()}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-6 md:p-8">
                    <div className="flex items-center gap-3">
                       <div className="p-2 bg-slate-100 rounded-lg text-slate-400 group-hover:text-brand-500 transition-colors">
                          <Briefcase size={16} />
                       </div>
                       <div>
                          <p className="text-sm font-black text-slate-900">{entry.description || 'General Mission'}</p>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Project: {entry.projectTitle || 'Internal'}</p>
                       </div>
                    </div>
                  </td>
                  <td className="p-6 md:p-8">
                    <div className="flex flex-col">
                       <span className="text-base font-black text-slate-900">{entry.hoursWorked}h</span>
                  ...
                  <td className="p-6 md:p-8">
                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border ${
                      ...
                      <span className="text-[9px] font-black uppercase tracking-[0.15em]">{entry.status}</span>
                    </div>
                  </td>
                    ...
                    {entry.status === 'Submitted' ? (
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          ...
                          className="p-2.5 bg-emerald-500/10 text-emerald-500 rounded-xl hover:bg-emerald-500 hover:text-white transition-all shadow-lg hover:shadow-emerald-500/20"
                        >
                          <Check size={18} />
                        </button>
                        <button 
                          onClick={() => handleAction(entry._id, 'Rejected')}
                          className="p-2.5 bg-rose-500/10 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-lg hover:shadow-rose-500/20"
                        >
                          <X size={18} />
                        </button>
                      </div>
...
const StatCard = ({ label, value, icon: Icon, color, bgColor }) => (
  <div className="glass-card p-6 md:p-8 flex items-center gap-6 group hover:border-brand-500/30 transition-all">
     <div className={`w-14 h-14 md:w-16 md:h-16 ${bgColor} rounded-[24px] flex items-center justify-center ${color} group-hover:scale-110 transition-transform`}>
        <Icon size={32} />
     </div>
     <div>
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{label}</p>
        <p className="text-2xl md:text-[36px] font-black text-slate-900 tracking-tighter leading-none">{value}</p>
     </div>
  </div>
);
