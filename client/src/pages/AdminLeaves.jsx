...
const AdminLeaves = () => {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-40 gap-4">
       ...
       <p className="text-white/40 font-black uppercase tracking-widest text-[10px]">Syncing Leave Protocols...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8 pb-20 max-w-7xl mx-auto">
       ...
       <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-[#020617]/40 p-8 rounded-[48px] border border-white/5 backdrop-blur-md">
          <div>
             <h1 className="text-4xl font-black text-white tracking-tighter">Leave Command</h1>
             <p className="text-white/40 mt-2 font-bold uppercase tracking-[0.2em] text-[10px]">Workforce Stability: {stats.teamAvailability} Availability Rate</p>
          </div>
          <div className="flex items-center gap-3">
             <div className="relative">
                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/20" size={14} />
                <input placeholder="Search requests..." className="w-64 pl-10 pr-4 py-2.5 rounded-2xl bg-white/5 border border-white/10 text-white text-xs outline-none focus:ring-2 focus:ring-brand-500/20" />
             </div>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 glass-light rounded-[40px] border border-white/5 overflow-hidden flex flex-col">
             <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/5">
                <div className="flex items-center gap-4">
                   ...
                   <h2 className="text-xl font-black text-white tracking-tight">Request Protocol Queue</h2>
                </div>
                <div className="flex bg-[#020617] rounded-xl p-1 border border-white/5">
                   {['pending', 'history'].map(tab => (
                      <button 
                        ...
                        className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-brand-600 text-white shadow-lg' : 'text-white/20 hover:text-white'}`}
                      >
                         {tab}
                      </button>
                   ))}
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar">
                {requests.filter(r => activeTab === 'pending' ? r.status === 'Pending' : r.status !== 'Pending').length === 0 ? (
                   <div className="flex flex-col items-center justify-center py-32 gap-4">
                      ...
                      <p className="text-white/20 font-black uppercase tracking-widest text-[10px]">No Active Requests in Queue</p>
                   </div>
                ) : (
                   <table className="w-full text-left border-collapse">
                      <thead>
                         <tr className="border-b border-white/5 bg-white/[0.02]">
                            ...
                            <th className="p-6 text-[10px] font-black text-white/30 uppercase tracking-[0.2em]">Duration</th>
                            <th className="p-6 text-[10px] font-black text-white/30 uppercase tracking-[0.2em]">Category</th>
                            ...
                            <tr key={idx} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                               <td className="p-6">
                                  <div className="flex items-center gap-4">
                                     ...
                                     <div>
                                        ...
                                        <p className="text-[9px] font-black text-white/20 uppercase tracking-widest">{req.role}</p>
                                     </div>
                                  </div>
                               </td>
                               <td className="p-6">
                                  <div className="flex flex-col">
                                     <span className="text-xs font-black text-white">{new Date(req.startDate).toLocaleDateString()}</span>
                               ...
                               <td className="p-6">
                                  <div className="flex items-center gap-2">
                                     ...
                                     <span className="text-[9px] font-black text-white uppercase tracking-widest">{req.type || 'Casual'}</span>
                                  </div>
                               </td>
                               <td className="p-6">
                               ...
                               <td className="p-6 text-right">
                                  {req.status === 'Pending' ? (
                                     <div className="flex items-center justify-end gap-2">
                                        <button onClick={() => handleAction(req._id, 'Approved')} className="p-2.5 bg-emerald-500/10 text-emerald-500 rounded-xl hover:bg-emerald-500 hover:text-white transition-all"><CheckCircle2 size={16} /></button>
                                        <button onClick={() => handleAction(req._id, 'Rejected')} className="p-2.5 bg-rose-500/10 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all"><XCircle size={16} /></button>
                                     </div>
                                  ) : (
          ...
          <div className="space-y-8">
             <div className="glass-light p-8 rounded-[40px] border border-white/5 relative overflow-hidden group">
                ...
                <h3 className="text-xl font-black text-white tracking-tight mb-2">Workload Analysis</h3>
                ...
                <div className="space-y-6">
                   ...
                   <div className="space-y-4">
                      <h4 className="text-[10px] font-black text-white/20 uppercase tracking-widest ml-1">Team Balance</h4>
                      {[
                        ...
                        { label: 'AI Core', val: 94, color: 'bg-indigo-500' }
                         ...
                         <div key={i} className="space-y-2">
                            <div className="flex justify-between text-[8px] font-black uppercase tracking-widest">
                               <span className="text-white/40">{dept.label}</span>
                               <span className="text-white">{dept.val}% Available</span>
                            </div>
             ...
             <div className="bg-slate-900 rounded-[40px] p-8 border border-white/10 relative overflow-hidden">
                <div className="flex items-center gap-3 mb-8">
                   ...
                   <div>
                      <h3 className="text-lg font-black text-white tracking-tight">Critical Alerts</h3>
                ...
                <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl">
                   ...
                   <p className="text-xs text-white/60 font-bold">Over 25% of Video Specialists are scheduled for leave next week.</p>
                </div>
                <button className="w-full py-4 bg-white/5 border border-white/5 rounded-2xl mt-6 text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white transition-all">Review Schedule</button>
             </div>
          </div>
       </div>
    </div>
